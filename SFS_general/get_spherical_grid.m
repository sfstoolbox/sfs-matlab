...
if strcmp('equally_spaced_points',spherical_grid)
    ...
    if ~exist(file,'file')
        % create dir
        mkdir([basepath '/data']);
        mkdir([basepath '/data/spherical_grids']);
        mkdir([basepath '/data/spherical_grids/equally_spaced_points']);
        % download file
        warning('Downloading grid file from http://github.com/sfstoolbox/data');
        urlwrite(url,file);
    end
    tmp = load(file,'-ascii');
    points = tmp(:,1:3);
