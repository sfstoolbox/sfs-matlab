...
x0 = SOFAcalculateAPV(irs); % / [deg deg m]
...
if norm(neighbours(:,1)-xs)<prec || ~useinterpolation
    ...
    ir = correct_radius(squeeze(irs.Data.IR(idx(1),:,:))',x0(idx(1),3),r,conf);
else
    % === IR interpolation ===
    ...
    if norm(neighbours(1,1)-neighbours(1,2))<prec || ...
        ...
        ir1 = correct_radius(squeeze(irs.Data.IR(idx(1),:,:))', ...
            x0(idx(1),3),r,conf);
        ir2 = correct_radius(squeeze(irs.Data.IR(idx(2),:,:))', ...
            x0(idx(2),3),r,conf);
        ...
        ir2 = correct_radius(squeeze(irs.Data.IR(idx(2),:,:))', ...
            x0(idx(2),3),r,conf);
        ir3 = correct_radius(squeeze(irs.Data.IR(idx(3),:,:))', ...
            x0(idx(3),3),r,conf);
