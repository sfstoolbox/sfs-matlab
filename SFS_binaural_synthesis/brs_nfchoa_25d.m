function brs = brs_nfchoa_25d(X,phi,xs,L,src,irs,conf)
%BRS_NFCHOA_25D generates a BRS set for use with the SoundScapeRenderer
%
%   Usage: brs = brs_nfchoa_25d(X,phi,xs,L,src,irs,[conf])
%
%   Input parameters:
...
%
%   BRS_NFCHOA_25D(X,phi,xs,L,irs,conf) prepares a BRS set for a virtual source
...
for ii = 1:length(angles)
    ...
    brs(:,(ii-1)*2+1:ii*2) = ...
        ir_nfchoa_25d(X,angles(ii)+phi,xs,L,src,irs,conf);
end
