function isargsecondarysourceforsphere(varargin)
%ISARGSECONDARYSOURCE tests if the given arg is a matrix containing secondary
%   source positions and directions for a spherical distribution using Minimum 
%   Energy Approach and return an error otherwise
%
%   Usage: isargsecondarysourceforsphere(args)
%
%   Input options:
%       args        - list of args
%
...
for ii = 1:nargin
    x0 = varargin{ii};
    if ~isnumeric(x0) || ndims(x0)~=2 || size(x0,2)~=8
        error(['%s need to be a nx8 matrix containing the secondary ', ...
            'sources positions and directions and the calculated weights'],inputname(ii));
    end
    for jj=1:size(x0,1)
