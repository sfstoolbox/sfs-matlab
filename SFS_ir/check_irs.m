...
if all(~isnumeric(irs.head_position)) || all(size(irs.head_position)~=[3 1])
    error('%s: head_position needs to be a 3x1 vector.',upper(mfilename));
elseif all(~isnumeric(irs.head_reference)) || all(size(irs.head_reference)~=[3 1])
     error('%s: head_reference needs to be a 3x1 vector.',upper(mfilename));
elseif all(~isnumeric(irs.source_position)) || all(size(irs.source_position,1)~=3)
     error('%s: source_position needs to be a 3xn vector.',upper(mfilename));
elseif all(~isnumeric(irs.source_reference)) || all(size(irs.source_reference,1)~=3)
