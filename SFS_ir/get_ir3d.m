function ir = get_ir3d(irs,phi,delta)
...
elseif findrows(roundto(irs.apparent_azimuth',prec),...
    ...
    delta = floor(degree(delta));
    phi = floor(degree(phi));
        
    idx = findrows(...
    [round(degree(irs.apparent_azimuth))' round(degree(irs.apparent_elevation))'],...
    [phi,delta]);
