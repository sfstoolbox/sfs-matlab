function isargdirectory(args,argnames)
%ISARGDIRECTORY tests if the given arg is a directory
%   Usage: isargdirectory(args,argnames)
%
%   Input options:
%       args        - list of args
%       argnames    - list of corresponding argnames (strings)
%
%   ISARGDIRECTORY(args,argnames) tests if all given args are a directory and
...
for ii = 1:length(args)
    if ~isnumeric(args{ii}) || ~exist(args{ii},'dir')
        error('%s need to be a directory.',argnames{ii});
    end
end
