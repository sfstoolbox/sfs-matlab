...
function [z,config] = hankel_rekursiv(config)
...
m = config.nLS./2; %Berechnung bis zur Ordnung m
a = sphbesselh(0,2,config.k2.*config.R); %Startwert 1: Hankelfunktion 0ter Ordnung
b = sphbesselh(1,2,config.k2.*config.R); %Startwert 2: Hankelfunktion 1ter Ordnung
z = forward(config,q,m,n,a,b); %Berechnung der Hankelfunktion
